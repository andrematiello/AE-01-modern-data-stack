...
| 3. Clean & organize | Renames, fixes types, computes returns, builds the star schema | **dbt** | Transformation as version-controlled SQL, so every change is auditable in Git |
...
1. **Ingestion (dlt):** incremental load of prices (merge on `ticker + date`) and ticker metadata into the
   warehouse, with no duplication on re-run.
...
`dbt build` runs clean on real market data, with **7 models, 22 tests, 0 errors**:
...
- `close_price`: **identical on all 5,000 rows**, so the transformation is deterministic across engines.
...
- **DuckDB (dev) + Snowflake (prod) on the same dbt project:** zero-cost local development, one target
  switch to a production cloud warehouse, demonstrating **warehouse portability**.
